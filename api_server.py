...
app.config['SESSION_COOKIE_SECURE'] = os.environ.get('ENV', 'production') == 'production'
    ...
    try:
        ...
        cursor.execute("""INSERT INTO arbi_trades (
            ...
            ON DUPLICATE KEY UPDATE
              ...
              fx_a_exit=COALESCE(VALUES(fx_a_exit),fx_a_exit),
              fx_b_exit=COALESCE(VALUES(fx_b_exit),fx_b_exit),
              fx_a_exit_ts=COALESCE(VALUES(fx_a_exit_ts),fx_a_exit_ts),
              fx_b_exit_ts=COALESCE(VALUES(fx_b_exit_ts),fx_b_exit_ts),
...
def start_background_threads():
    ...
    def _nightly_audit_thread():
        ...
        def _snap_arbi():
            # [ARBI-AUDIT 18-jul] leitura pura, zero interferencia no engine
            with state_lock:
                return list(arbi_closed)
        _naud({'log': log, 'beat': beat, 'get_closed_snapshot': _snap,
               'get_arbi_snapshot': _snap_arbi,
    ...
    try:
        ...
        if request.args.get('run') == '1':
            ...
            return jsonify(run_nightly_audit({'log': log, 'beat': lambda *_: None,
                'get_closed_snapshot': _snap, 'get_arbi_snapshot': _snap_arbi,
